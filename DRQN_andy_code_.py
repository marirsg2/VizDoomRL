...
from keras.layers import Dense, Activation, Conv2D, Input, Flatten, LSTM, Reshape, RepeatVector, TimeDistributed, Convolution2D
...
def create_model(available_actions_count):
    ...
    model.add(Dense(output_dim=available_actions_count, activation='linear'))
    ...
    # k_model.add(Conv2D(filters=8,kernel_size=[3,3], strides=[2,2], activation='relu'))
    # k_model.add(Flatten())
    # k_model.add(LSTM(4, batch_input_shape=(batch_size, look_back, 96), stateful=True))
    #
